#!/bin/bash
#SBATCH --job-name=pileup
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=frans.budiman@alumni.utoronto.ca
#SBATCH --time=24:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --array=1-66
#SBATCH --output=/scratch/frans/rna-seq/logs/pileup_%A_%a.out
#SBATCH --error=/scratch/frans/rna-seq/logs/pileup_%A_%a.err

module load StdEnv/2023
module load gatk

WORK_DIR="/scratch/frans/rna-seq"
cd ${WORK_DIR}
source ${WORK_DIR}/resources/resource_paths_with_rg.sh

PAIR_LINE=$(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" ${SAMPLE_FILE})
IFS=$'\t' read -r TUMOR_BAM NORMAL_BAM TUMOR_NAME NORMAL_NAME PAIR_ID <<< "${PAIR_LINE}"

OUTPUT_DIR="${WORK_DIR}/results/pileup_summaries"
mkdir -p ${OUTPUT_DIR}

echo "Step 3: Pileup Summaries (RNA-seq) - ${PAIR_ID}"
echo "Start: $(date)"

# Tumor pileups - disable mapping quality filters for RNA-seq
gatk GetPileupSummaries \
    -I ${TUMOR_BAM} \
    -V ${GNOMAD} \
    -L ${INTERVALS} \
    --disable-read-filter MappingQualityAvailableReadFilter \
    --disable-read-filter MappingQualityReadFilter \
    --disable-read-filter MappingQualityNotZeroReadFilter \
    -O ${OUTPUT_DIR}/${PAIR_ID}_tumor_pileups.table

# Normal pileups - disable mapping quality filters for RNA-seq  
gatk GetPileupSummaries \
    -I ${NORMAL_BAM} \
    -V ${GNOMAD} \
    -L ${INTERVALS} \
    --disable-read-filter MappingQualityAvailableReadFilter \
    --disable-read-filter MappingQualityReadFilter \
    --disable-read-filter MappingQualityNotZeroReadFilter \
    -O ${OUTPUT_DIR}/${PAIR_ID}_normal_pileups.table

echo "End: $(date)"
